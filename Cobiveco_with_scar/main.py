...
def main():
    ...
    dim_br_geo =  [60, 200, 200, 200, 200, 200, 200]
    dim_br_scar = [60, 200, 200, 200, 200, 200, 200]
    dim_br_pace = [4, 200, 200, 200, 200, 200, 200] 
    dim_tr =      [4, 200, 200, 200, 200, 200, 200]

    batch_size = 10
    learning_rate = 0.0005
    
    save_directory = f'scar_SDF_separate_{epochs}ep_{learning_rate}lr'
    ...
    data_path = "./DIMON_training_data_scar_SDF.npz"
    ...
    f_train = geo_input[:num_train_hearts]
    s_train = scar[:num_train_hearts]
    u_train_raw = u_all[:num_train_hearts]
    
    f_val = geo_input[num_train_hearts:num_total_train]
    s_val = scar[:num_train_hearts:num_total_train]
    u_val_raw = u_all[num_train_hearts:num_total_train]
    
    f_test = geo_input[num_total_train:]
    s_test = scar[num_total_train:]
    ...
    f_train_norm = (f_train - f_mean) / f_std # Add small constant to avoid zero division
    f_val_norm = (f_val - f_mean) / f_std
    f_test_norm = (f_test - f_mean) / f_std

    s_mean, s_std = s_train.mean(axis=0), s_train.std(axis=0)
    ...
    s_train_tensor = torch.tensor(s_train_norm, dtype=torch.float).to(device)
    s_val_tensor = torch.tensor(s_val_norm, dtype=torch.float).to(device)
    s_val_tensor = torch.tensor(s_val_norm, dtype=torch.float).to(device)
    s_test_tensor = torch.tensor(s_test_norm, dtype=torch.float).to(device)
    s_test_tensor = torch.tensor(s_test_norm, dtype=torch.float).to(device)

    model = opnn(dim_br_geo, dim_br_scar, dim_br_pace, dim_tr).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=1e-5)

    if test_model == 0:
            print(f"--- Starting Training (Batch Size: {batch_size} hearts) ---")
            train_ds = TensorDataset(f_train_tensor, s_train_tensor, u_train_tensor)
            train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True)
            
            train_loss_his = []
            test_loss_his = []  # Replaced val_loss_his with test_loss_his
            ...
            for epoch in range(epochs):
                model.train()
                epoch_loss = 0
                for b_f, b_s, b_u in train_loader:
                    optimizer.zero_grad()
                    y_pred = model.forward(b_f, b_s, x_pace_tensor, x_tensor)
                ...
                model.eval()
                with torch.no_grad():
                    # Validation loss for saving best weights
                    y_val = model.forward(f_val_tensor, s_val_tensor, x_pace_tensor, x_tensor)
                    val_loss = ((y_val - u_val_tensor)**2).mean().item()
                    
                    # Test loss for history tracking
                    y_test = model.forward(f_test_tensor, s_test_tensor, x_pace_tensor, x_tensor)
            ...
            np.savetxt(f'./Predictions/{save_directory}/test_loss.txt', np.array(test_loss_his)) # Save test loss history        
        ...
        num_sims = 9        # All simulations by pacing
            
        with torch.no_grad():
            # Get Predictions for Test Set
            y_pred_test_raw = model.forward(f_test_tensor, s_test_tensor, x_pace_tensor, x_tensor)
            ...
            s_train_subset = s_train_tensor[:num_viz_hearts]
            y_pred_train_raw = model.forward(f_train_subset, s_train_subset, x_pace_tensor, x_tensor)
            u_train_pred = to_numpy(y_pred_train_raw) * u_std + u_mean_train
                
            u_train_phy = u_train_raw[:num_viz_hearts]
            u_test_phy_viz = u_test_raw[:num_viz_hearts] # Used strictly for 3D viz
            ...
            def plot_full_comparison(pred_batch, true_batch, folder_name, file_prefix):
                ...
                for i in range(num_viz_hearts):
                    # Loop through all pacing simulations for that heart
            ...
            u_test_phy_full = u_test_raw # Use entire test set for distributions
            records = []
