...
def main():
    ...
    dim_tr =      [12, 200, 200, 200, 200]

    batch_size = 10
    learning_rate = 0.0005
    
    save_directory = f'cobiveco_{epochs}ep_{learning_rate}lr'
    ...
    anisotropy = dataset['ref_anisotropy'] # (N, 9)
    x_coords = dataset['cartesian_coords'] # (N, 3)

    trunk_input = np.hstack([x_coords, anisotropy]) # (N, 12)
    # trunk_input = cobiveco

    f_train = theta[:num_train_hearts]
    u_train_raw = u_all[:num_train_hearts]
    
    f_val = theta[num_train_hearts:num_total_train]
    ...
    if test_model == 0:
            ...
            lambda_pde = 0.1  # Set PDE loss weight coefficient
            for epoch in range(epochs):
                ...
                for b_f, b_u in train_loader:
                    ...
                    ##########################################################################################################################
                    loss = ((y_pred - b_u)**2).mean() # data loss only, no PDE residuals in this implementation
                    ##########################################################################################################################
                    loss.backward()
                    optimizer.step()
