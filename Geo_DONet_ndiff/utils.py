...
def ParseArgument():
    ...
    parser.add_argument('--ndiff-weight', type=float, default=0.0,
                        help='weight lambda on the signed-Laplacian neighbour-'
                             'difference loss (0 = baseline MSE only). Total loss '
                             '= MSE + lambda * mean((L(pred-gt))^2), L = I - D^-1 A.')
    parser.add_argument('--adj-file', type=str,
                        default=os.environ.get(
                            'DIMON_ADJ_FILE',
                            '/home/svu/e1032484/scratch/reference/'
                            'mesh_adjacency_data_order.npz'),
                        help='canonical-order mesh adjacency npz (edge_src/dst, '
                             'n_nodes) for the neighbour-difference loss')
