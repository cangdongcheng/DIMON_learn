...
DATA_FILE = os.environ.get("DIMON_DATA_FILE", "geo_donet_data_f121.npz")


def load_cartesian_coords():
    """Canonical-reference Cartesian xyz (50797, 3), in npz `coords` node order.

    Prefers the upstream DIMON_training_data_healthy.npz at the repo root; on
    Vanda (where that file isn't staged) falls back to canonical_xyz.npz, which
    scratch_scripts/extract_canonical_xyz.py extracts from canonical.vtu after
    verifying its ab/rt/tm/tv match the training-npz Cobiveco row-for-row.
    Override the fallback path with DIMON_CARTESIAN_NPZ.
    """
    primary = os.path.join(os.path.dirname(__file__), "..",
                           "DIMON_training_data_healthy.npz")
    if os.path.exists(primary):
        return np.load(primary)["cartesian_coords"].astype(np.float32)
    fallback = os.environ.get(
        "DIMON_CARTESIAN_NPZ",
        os.path.join(DATA_BASE, "canonical_xyz.npz"))
    if os.path.exists(fallback):
        print(f"cartesian_coords: using fallback {fallback}")
        return np.load(fallback)["cartesian_coords"].astype(np.float32)
    raise FileNotFoundError(
        f"Need cartesian_coords but neither {primary} nor {fallback} exists. "
        f"Run scratch_scripts/extract_canonical_xyz.py or stage "
        f"DIMON_training_data_healthy.npz.")
...
def main():
    ...
    # save_directory + paths defined below, after subsampling fixes the frame count.

    ## Load Dataset
    data_path = os.path.join(DATA_BASE, DATA_FILE)
    if not os.path.exists(data_path):
        print(f"ERROR: {data_path} not found.")
        print("Override via DIMON_DATA_BASE / DIMON_DATA_FILE env vars, or run:")
        print("  python package_training_data.py --frame-step <step>")
        return
    ...
    if args.trunk == 'xyz':
        coords = load_cartesian_coords()  # (50797, 3)
        print("Trunk: 3D Cartesian xyz")
    else:
    ...
    # Frame-count tag reflects the *post-subsample* count so derived runs
    # (e.g. f601 → f301 via --frame-step 2) get a checkpoint path matching
    # what package_training_data.py would have written.
    frames_tag = f'_f{n_time}'
    ndiff_tag = f'_ndiff{args.ndiff_weight:g}' if args.ndiff_weight > 0 else ''
    save_directory = f'geo_donet_siren_{epochs}ep_w{width}{omega_tag}{lr_tag}{trunk_tag}{ndiff_tag}{frames_tag}'
    ...
    if test_model == 0:
        ...
        # Signed-Laplacian neighbour-difference loss (see Geo_DONet_ndiff). On the
        # SIREN trunk this tests whether a high-frequency-capable net can actually
        # reduce the spatial neighbour-diff error the Tanh trunk could not.
        ...
        with torch.no_grad():
            ...
            if args.skip_snapshots:
                ...
                cartesian_coords = load_cartesian_coords()
