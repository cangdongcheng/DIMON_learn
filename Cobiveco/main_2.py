...
def main():
    ...
    if test_model == 1:
        load_path = "./CheckPts/model_chkpts_baseline.pt"
    ...
    ## create folders
    dump_test = './Predictions/Test_Baseline_cobiveco/'
    model_path = 'CheckPts/model_chkpts_baseline.pt'
    ...
    x_tensor = torch.tensor(cobiveco, dtype=torch.float).to(device)
    ...
    if test_model == 0:
        ...
        np.savetxt('./Predictions/Test_Baseline/train_loss.txt', train_loss)
        np.savetxt('./Predictions/Test_Baseline/val_loss.txt', val_loss_his)
